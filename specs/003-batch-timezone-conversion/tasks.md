...
- [X] T030a [P] [US2] Integration test: SYS-009 output-write failure after SYS-006 has processed all rows — orchestrator surfaces the write failure (exit code 3) rather than crashing or discarding results, in `tests/TimezoneUtility.Integration/Commands/ConvertBatchCommandTests.cs` (`STS_011_A2_OutputWriteFails_OrchestratorSurfacesFailure_ExitCode3_AfterAllRowsProcessed`) — verifies REQ-018, REQ-029; corresponds to STP-011-A (STS-011-A2). Added during re-verification after System Testing flagged STS-011-A2 as having no executable test.
...
## Follow-up: close the 9 STS gaps flagged by System Testing's reconcile_evidence pass (2026-09-21)

- [X] T046 Fix real orchestrator gap: `ConvertBatchCommand.Execute`'s per-row loop had no try/catch, so an unhandled exception escaping `BatchRowProcessor.Process` (e.g. a throw from within its own catch block's `InvalidRowReporter.Report` call) would crash the whole run instead of being isolated by SYS-011 per the SYS-011→SYS-006 dependency edge in system-design.md. Extracted the loop into new `Services/BatchConversion/BatchRowsOrchestrator.ProcessAll(...)` (testable via an injectable `processRow` delegate) with its own try/catch around each row's call, and wired `ConvertBatchCommand` to use it. Verifies REQ-006, REQ-011, REQ-NF-001; corresponds to STP-006-B (STS-006-B1). Test: `BatchRowsOrchestratorTests.STS_006_B1_UnhandledExceptionFromRowTwo_OrchestratorContinuesToRowsThreeAndFour`.
- [X] T047 [P] Add `STS_001_A2_FileExistsButUnreadable_ReturnsFileLevelUnreadableError_NoRowStreamProduced` and `STS_001_B1_SYS001RaisesFileLevelError_SYS011AbortsImmediately_SYS002AndSYS006NeverInvoked` to `ConvertBatchCommandTests.cs` — uses an exclusive OS-level file lock (permission bits are ignored running as root in this environment) to force the same `IOException` path `CsvBatchReader` already catches, then asserts exit code 2 and both output artifacts absent. Verifies REQ-001, REQ-016, REQ-018; corresponds to STP-001-A/STP-001-B (STS-001-A2, STS-001-B1).
- [X] T048 [P] Add `STS_008_B1_300ValidRows_ProducesExactly300SuccessfulOutputRecords_ZeroMissing` to `ConvertBatchCommandTests.cs` (300-row all-valid batch boundary). Verifies REQ-007, REQ-NF-001; corresponds to STP-008-B (STS-008-B1).
- [X] T049 [P] Add dedicated system-scope (non-acceptance-named) tests carrying only `STS_*` identifiers, since the traceability matrix's own convention treats an `ATP_*`-named integration test as a Matrix A artifact only, not a Matrix B/STS match, even when it exercises the same behavior: `TimeServiceDstBoundaryTests.STS_005_B1_...`/`STS_005_B2_...` (SYS-005 DST boundary, unit-level, direct `TimeService.ConvertTime` calls) and `ConvertBatchCommandTests.STS_011_A1_...`/`STS_011_B2_...` (SYS-011 column-resolution-abort and 10,000-row single-pass, orchestrator-level). Also renamed the pre-existing `ATP_013_A_SCN_013_A1_*`, `ATP_026_A/B_SCN_026_*`, `ATP_017_A_SCN_017_A1_*`, and `ATP_NF_005_A_SCN_NF_005_A1_*` test methods to additionally embed their corresponding `STS_003_A1`, `STS_005_B1/B2`, `STS_011_A1`, `STS_011_B2` tokens for traceability, even though the dedicated pure-STS tests above are the authoritative Matrix B match. Verifies REQ-003/008-015 (STS-003-A1), REQ-026/REQ-NF-004 (STS-005-B1/B2), REQ-018/REQ-029 (STS-011-A1), REQ-023/REQ-NF-005 (STS-011-B2).

All 9 STS ids flagged by the 2026-09-21 reconcile_evidence pass (STS-001-A2, STS-001-B1, STS-003-A1, STS-005-B1, STS-005-B2, STS-006-B1, STS-008-B1, STS-011-A1, STS-011-B2) now have an executable test. Full suite: 87/87 passing (Unit 48, Contract 7, Integration 32).
